 ...
 * @method     ChildPatientQuery orderByUserId($order = Criteria::ASC) Order by the user_id column
 *
 * @method     ChildPatientQuery groupById() Group by the id column
 * @method     ChildPatientQuery groupByUserId() Group by the user_id column
 *
 ...
 *
 * @method     ChildPatientQuery leftJoinNewuser($relationAlias = null) Adds a LEFT JOIN clause to the query using the Newuser relation
 * @method     ChildPatientQuery rightJoinNewuser($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Newuser relation
 * @method     ChildPatientQuery innerJoinNewuser($relationAlias = null) Adds a INNER JOIN clause to the query using the Newuser relation
 *
 * @method     ChildPatientQuery joinWithNewuser($joinType = Criteria::INNER_JOIN) Adds a join clause and with to the query using the Newuser relation
 *
 * @method     ChildPatientQuery leftJoinWithNewuser() Adds a LEFT JOIN clause and with to the query using the Newuser relation
 * @method     ChildPatientQuery rightJoinWithNewuser() Adds a RIGHT JOIN clause and with to the query using the Newuser relation
 * @method     ChildPatientQuery innerJoinWithNewuser() Adds a INNER JOIN clause and with to the query using the Newuser relation
 *
 ...
 * @method     ChildPatient findOneByUserId(int $user_id) Return the first ChildPatient filtered by the user_id column *
 ...
 * @method     ChildPatient requireOneByUserId(int $user_id) Return the first ChildPatient filtered by the user_id column and throws \Propel\Runtime\Exception\EntityNotFoundException when not found
 *
 ...
 * @method     ChildPatient[]|ObjectCollection findByUserId(int $user_id) Return ChildPatient objects filtered by the user_id column
...
abstract class PatientQuery extends ModelCriteria
{
    ...
    {
        $sql = 'SELECT id, user_id FROM Patient WHERE id = :p0';
        try {
    ...
    public function filterByUserId($userId = null, $comparison = null)
    {
        if (is_array($userId)) {
            $useMinMax = false;
            if (isset($userId['min'])) {
                $this->addUsingAlias(PatientTableMap::COL_USER_ID, $userId['min'], Criteria::GREATER_EQUAL);
                $useMinMax = true;
            }
            if (isset($userId['max'])) {
                $this->addUsingAlias(PatientTableMap::COL_USER_ID, $userId['max'], Criteria::LESS_EQUAL);
                $useMinMax = true;
            }
            if ($useMinMax) {
                return $this;
        ...
        return $this->addUsingAlias(PatientTableMap::COL_USER_ID, $userId, $comparison);
    }
    ...
    public function filterByNewuser($newuser, $comparison = null)
    {
        if ($newuser instanceof \Newuser) {
            return $this
                ->addUsingAlias(PatientTableMap::COL_USER_ID, $newuser->getId(), $comparison);
            ...
            return $this
                ->addUsingAlias(PatientTableMap::COL_USER_ID, $newuser->toKeyValue('PrimaryKey', 'Id'), $comparison);
        } else {
